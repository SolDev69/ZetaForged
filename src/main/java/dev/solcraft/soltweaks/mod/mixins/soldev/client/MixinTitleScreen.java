package dev.solcraft.soltweaks.mod.mixins.soldev.client;
...
import static dev.solcraft.soltweaks.mod.SolTweaks.*;
...
public class MixinTitleScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta, CallbackInfo ci) {
        ...
        if((Date.from(Instant.now()).getDate() == 16 && Date.from(Instant.now()).getMonth() == Calendar.MARCH)
            ...
            drawStringWithShadow(matrices, this.textRenderer, "HAPPY BIRTHDAY SOLTWEAKS!", 2, 2, 0xffff00 | l);
