package dev.solcraft.soltweaks.mod;
...
import dev.solcraft.soltweaks.mod.features.biomes.BiomesInitializer;
import dev.solcraft.soltweaks.mod.features.blocks.NegativeSlipperinessBlock;
import dev.solcraft.soltweaks.mod.features.items.SuperFireballItem;
import dev.solcraft.soltweaks.mod.managers.commands.CommandsManager;
import dev.solcraft.soltweaks.mod.features.errors.compute.ComputeErrorFunction;
import dev.solcraft.soltweaks.mod.features.items.ConcernedTater;
...
import dev.solcraft.soltweaks.mod.managers.ConfigManager;

public class SolTweaks implements ModInitializer {

    private static Logger LOGGER = LogManager.getLogger();
	public static final String MOD_ID = "soltweaks";
	public static final String MOD_NAME = "SolTweaks";
	...
	public static final ConcernedTater CONCERNED_TATER = new ConcernedTater(new FabricItemSettings().group(SolTweaks.SOLTWEAKS_ITEMS));
	//public static final Item EXAMPLE_ITEM = Registry.register(Registry.ITEM,new Identifier("mymodid","example_item"), new Item(new FabricItemSettings().group(zeta.soltweaks.mod.SolTweaks.SOLTWEAKS_ITEMS)));

	public static final Level LV = Level.INFO;
	public static double concerning_weight = Math.pow(2, -4);

	public static final ItemGroup SOLTWEAKS_ITEMS = FabricItemGroupBuilder.create(
			new Identifier(MOD_ID,"soltweaks_group")).icon(
	...
	public void onInitialize() {
		...
		setModVersion();
		ZFVersionString = "SolTweaks " + SolTweaks.MOD_VERSION;
		log2("Initializing SolTweaks");
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, "op_fireball"), new SuperFireballItem(new FabricItemSettings().group(SOLTWEAKS_ITEMS)));
		...
		Registry.register(Registry.ITEM, new Identifier(MOD_ID, "gigaconcern_block"), new BlockItem(GIGACONCERN_BLOCK, new Item.Settings().group(SolTweaks.SOLTWEAKS_ITEMS)));

		Registry.register(Registry.ITEM, new Identifier(MOD_ID,"concerned_tater"), CONCERNED_TATER);
		//SolTweaks.log(Level.INFO, "Patching Farlands!");
